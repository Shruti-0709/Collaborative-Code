...
function EditorPage() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            // Fetch initial code from MongoDB when the room is joined
            try {
                const res = await axios.get(`http://localhost:5001/code/${roomId}`);
                codeRef.current = res.data.code || ""; // Store initial code locally
            } catch (err) {
                console.error("Failed to load initial code:", err);
                toast.error("Failed to load code from database");
            }
                ...
                <div className={showChat ? "col-md-7 d-flex flex-column" : "col-md-10 d-flex flex-column"}>
                    <div style={{ height: 'calc(80vh - 60px)', overflow: 'auto', padding: '10px', border: '1px solid #333' }}>
                        <Editor socketRef={socketRef} roomId={roomId} onCodeChange={(code) => {
                            ...
                            // Save the code to MongoDB whenever it changes
